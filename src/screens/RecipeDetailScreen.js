...
export default function RecipeDetailScreen(props) {
    ...
    const checkIfBookmarked = () => {
        setIsFavourite(user.bookmarks.includes(item.idMeal));
    };

    const toggleBookmark = () => {
        if (isFavourite) {
            removeBookmark(item.idMeal);
        } else {
            addBookmark(item.idMeal);
        }
            ...
            <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 30 }}>
                ...
                <Animated.View entering={FadeIn.delay(200).duration(1000)} className="w-full absolute flex-row justify-between items-center pt-14">
                    <TouchableOpacity onPress={() => navigation.goBack()} className="p-2 rounded-full ml-5 bg-white">
                        <ChevronLeftIcon size={hp(3.5)} strokeWidth={4.5} color="#60A5FA" />
                    </TouchableOpacity>
                    <TouchableOpacity onPress={toggleBookmark} className="p-2 rounded-full mr-5 bg-white">
